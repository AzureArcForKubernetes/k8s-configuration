Describe 'Flux Configuration (OCI Repository - Workload Identity) Testing' {
    ...
    It 'Creates a configuration with Workload Identity enabled on the cluster' {
        $output = az k8s-configuration flux create -c $ENVCONFIG.arcClusterName -g $ENVCONFIG.resourceGroup --cluster-type "connectedClusters" -n $configurationName --namespace $configurationName --scope cluster --kind oci -u $url --tag $tag --use-workload-identity --kustomization name=workloadtest path=./ prune=true --no-wait
        $? | Should -BeTrue
