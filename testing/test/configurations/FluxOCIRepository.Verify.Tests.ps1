Describe 'Flux Configuration (OCI Repository - Verification) Testing' {
    ...
    It "Updates configuration with multiple OIDC identities" {
        # Test with multiple OIDC identities
        $multipleIdentities = @(
        ...
        $output = az k8s-configuration flux update -c $ENVCONFIG.arcClusterName -g $ENVCONFIG.resourceGroup --cluster-type "connectedClusters" -n $configurationName --kind oci --verification-provider "cosign" --match-oidc-identity $multipleIdentities --no-wait
        $? | Should -BeTrue
        ...
        do 
        {
            ...
            # Check multiple OIDC identities
            $verifyElement = $jsonOutput.RootElement.GetProperty("ociRepository").GetProperty("verify")
            ...
            if ($provisioningState -eq $SUCCEEDED -and $identityCount -eq 2) {
